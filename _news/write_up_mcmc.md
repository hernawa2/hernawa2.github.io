...
That is it for MCMC. The example I show above is pedagogical and in real-application is, more-often than not, always more complex. In high-dimension problem (the number of parameters $\theta$ is a lot), MCMC may take a very long time to reach the posterior distribution. If you ponder about it, in two-dimensional space, walking randomly will almost surely get you to where you want to go. But in dimension higher than that, unguided random walk stretches where the new point is, one parameter might want to go to the left, others go to the right, while other parameters want to go up to reach the target.
